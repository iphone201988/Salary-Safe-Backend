...
@router.post("/request_demo", response_model=Message)
def request_demo(session: SessionDep, user_in: RequestDemoBase) -> Any:
    """
    Request a demo
    """
    # Check if the user already exists in the system
    system_user = crud.get_user_by_email(session=session, email=user_in.email)
    if system_user:
        raise HTTPException(
            status_code=400,
            detail="The user with this email already exists in the system",
    ...
    if request_user:
        raise HTTPException(
            status_code=400,
            detail="Request already submitted with this email",
        )
